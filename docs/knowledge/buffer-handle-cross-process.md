...
`SurfaceBufferImpl` 支持注册 buffer 析构回调（入参为 `bufferId_`），支持多个使用方各自注册：内部用 `bufferDtorCbs_`（`pair<函数指针, std::function>` 向量）保存，析构时在锁外按注册顺序依次回调，锁外回调是为了避免回调重入本类接口时死锁。注册分两套接口：`RegisterBufferDestructorCallback(std::function)` 不带身份，既不去重也无法单独注销；`RegisterBufferDestructorCallbackFunc(void(*)(uint64_t))` 以函数指针作为身份，重复注册同一函数会被忽略，并可用 `UnRegisterBufferDestructorCallbackFunc` 精确注销。不把它们做成同名重载有两个硬约束：本仓以 `-fno-rtti` 编译，libc++ 在该模式下不提供 `std::function::target()/target_type()`，无法从 std::function 反查回调身份；而同名的 `std::function` 与函数指针重载会让无捕获 lambda 的调用变成 ambiguous 编译错误，直接破坏下游源码兼容。新增虚函数必须追加在 `SurfaceBuffer` public 虚函数列表末尾（当前在 `GetAndResetSingleBufferMode` 之后），插在中间会移动其后所有虚函数的 vtable 槽位，让按旧头文件编译的二进制调到错误函数。无参 `UnRegisterBufferDestructorCallback()` 清空全部回调，会连带清掉其它模块的注册，多使用方场景应用 `...Func` 版本；曾考虑把它收窄成只清身份为 `nullptr` 的项（改动前只有一个槽，无参注销事实上等于注销自己那一个，收窄更贴近原语义），但全树 grep 确认老接口注册与无参注销都已无调用方，最终决定不改语义、只在头文件注释和实现处写明影响面。残留的两条未通知路径要记住：老接口不去重，重复注册会累积并挤占 `MAX_BUFFER_DTOR_CB_NUM` 名额，超限的注册被静默丢弃；仓外若有只 override 老虚函数的 `SurfaceBuffer` 派生类或 mock，调用方改用 `...Func` 会落到基类空实现，同样静默不通知。回调总数上限为 `MAX_BUFFER_DTOR_CB_NUM`（16），防止重复注册导致向量无限增长。当前使用方是 graphic_2d 的 `RSBufferReclaim`（reclaim 时用 `RegisterBufferDestructorCallbackFunc` 注册、resume 时用 `UnRegisterBufferDestructorCallbackFunc` 按函数指针精确注销）和 `RsDelegateCompositeCallbackManager`（清缓存时用 `RegisterBufferDestructorCallbackFunc` 注册、不注销），两者都已迁移到带身份的 `...Func` 接口，因此 reclaim/resume 不会再连带清掉 delegate composite 的注册。仓内已无 `RegisterBufferDestructorCallback(std::function)` 的调用方，该接口保留给带捕获 lambda 的使用方，代价是这类注册无法去重也无法单独注销。
