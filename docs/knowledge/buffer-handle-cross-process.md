...
`SurfaceBufferImpl` 支持注册 buffer 析构回调（入参为 `bufferId_`），支持多个使用方各自注册：内部用 `bufferDtorCbs_`（`pair<函数指针, std::function>` 向量）与单槽成员 `bufferDtorCb_`（`std::function<void(uint64_t)>`）分别保存两套接口的注册，两套存储物理隔离、互不挤占名额；析构时在锁外先按注册顺序依次回调向量里的、再回调单槽里的，锁外回调是为了避免回调重入本类接口时死锁；登记表是在锁内用 `swap` 整体转移给局部变量后再逐个调用的（单槽也在同一临界区内一并 `std::move` 出来），而不是逐个拷贝 `std::function`（带捕获的 lambda 每次拷贝构造都要堆分配，上限提到 32 后锁内开销随之翻倍），转移把锁内工作降为一次指针交换。`swap` 要求两侧类型完全一致，所以局部变量必须声明为与 `bufferDtorCbs_` 相同的 `pair` 向量，`vector<std::function>` 与 `vector<pair>` 之间无法 swap；`NotifyBufferDestructorCallback` 也因此去掉了 `const`（`std::vector::swap` 是非 const 成员函数，而 `bufferDtorCbs_` 不是 mutable，本仓 6 处 mutable 全部只用于 mutex），它是 private、非虚、唯一调用点是析构函数，不进 vtable 也不影响任何调用方，故无 ABI 影响。两套一起取出正是重入用例（单测 011）成立的前提：回调在通知期间改不动正在被通知的那一批。转移语义相对快照的唯一可观察差异是：回调在通知期间反注册会因登记表已空而返回 false（单测 011 已把这两个返回值显式钉住）。生产上打不到这条差异——`RSBufferReclaim::BufferDestructorCallback` 只调 `RemoveBufferReclaim(bufferId)`、`RsDelegateCompositeCallbackManager::BufferDestructorCallback` 只调 `ProcessBufferReleaseInfo(bufferId)`，两者都只收 `uint64_t`、只动自己单例的状态，拿不到正在析构的 buffer，因而无法重入登记表；两处 `UnRegisterBufferDestructorCallbackFunc` 都在 `DoBufferReclaim`/`DoBufferResume` 的正常 API 路径上，持有存活的 `sptr<SurfaceBuffer>`。注册分两套接口，且各自有独立存储：`RegisterBufferDestructorCallback(std::function)` 不带身份，既不去重也无法单独注销，存于单槽 `bufferDtorCb_`，槽位被占用后的注册静默丢弃（first-wins）；`RegisterBufferDestructorCallbackFunc(void(*)(uint64_t))` 以函数指针作为身份，重复注册同一函数会被忽略，并可用 `UnRegisterBufferDestructorCallbackFunc` 精确注销，存于向量登记表 `bufferDtorCbs_`。两个 `...Func` 接口都返回 `bool`：注册成功、或同一函数重复注册（幂等）返回 true，回调为 null、超过上限、落到基类空实现返回 false；注销命中返回 true，未命中返回 false（未命中同时说明该注册从未生效，例如注册时被上限丢弃）。不把它们做成同名重载有两个硬约束：本仓以 `-fno-rtti` 编译，libc++ 在该模式下不提供 `std::function::target()/target_type()`，无法从 std::function 反查回调身份；而同名的 `std::function` 与函数指针重载会让无捕获 lambda 的调用变成 ambiguous 编译错误，直接破坏下游源码兼容。新增虚函数必须追加在 `SurfaceBuffer` public 虚函数列表末尾（当前在 `GetAndResetSingleBufferMode` 之后），插在中间会移动其后所有虚函数的 vtable 槽位，让按旧头文件编译的二进制调到错误函数。无参 `UnRegisterBufferDestructorCallback()` 只把单槽 `bufferDtorCb_` 置回 `nullptr`，实现里没有循环（这个接口从来只有一个槽位），`...Func` 的注册全部保留、仍会收到析构通知，因此它不会连带清掉其它模块用 `...Func` 挂上的回调；它也是移除老接口注册的唯一途径。存储物理隔离带来的直接收益是两个接口互不挤占：`...Func` 填满 32 个槽位后老接口仍能注册成功，老接口占着自己的槽位也不消耗 `...Func` 的名额，`MAX_BUFFER_DTOR_CB_NUM`（32）这个上限完整归 `...Func`，因此不需要配额常量、不需要 `static_assert` 防 `uint32_t` 相减下溢，也没有「`...Func` 可用容量被收窄」这条对外可见的代价。通知顺序是先向量（按注册顺序）后单槽；生产上老接口零调用方，两套之间的先后在实际运行中不可观察，且对每个 buffer 都一致。这里有过两次决策演进：最初全树 grep 确认老接口注册与无参注销都零调用方，判定收窄无现实收益却扩大行为变更面，决定保留 master 的 `clear()` 清空全部的语义、只在头文件注释和实现处写明影响面；第一轮评审后改为收窄，理由是无参注销连带清掉他人注册属于只靠文档提示的陷阱，而收窄是纯行为变更、签名不动因而不构成 ABI 破坏，且生产调用方为零意味着仓内没有回归面，当时把老接口注册也塞进同一张向量、用 `nullptr` 当类别标记；第二轮评审要求「老接口单列一个小配额」，曾按 `LEGACY_BUFFER_DTOR_CB_NUM`（4）预留配额实现，最终整体回退为「老接口保留 master 的单槽与 first-wins、两套存储物理隔离、只在通知处兼容新老」。回退的理由是这让实现回到 master 形态，两条评审意见（erase 后 `break`、预留配额）指向的评审点自然不存在，`surface_buffer_impl.cpp` 相对 master 的 diff 从 4 个 hunk 缩到 2 个（两个构造函数的 hunk 完全消失），还省掉了配额常量、`static_assert`、`nullptr` 类别标记的整套推理和「一次调用只移除一条」这条别扭的对外契约。必须接受的代价是老接口不解决多端注册——两个模块都走老接口时第二个被丢弃且返回 `void` 无从告知，这是 issue 26235 的原始痛点；但 graphic_2d 的两个调用方都已迁移到 `...Func`，老接口生产零调用方，需求仍被满足且无回归面。这个局限由老接口签名（注册返回 `void`、注销无入参）没有身份通道导致，不改签名就无法消除。曾评估过给老接口注册生成合成函数指针作为身份、塞进同一张登记表，但合成值既无法回传给调用方（注册返回 `void`）也无法由调用方传入（注销无入参），不增加任何能力，反而引入合成值与真实函数地址碰撞导致 `UnRegisterBufferDestructorCallbackFunc` 误删的风险，故未采纳；真要按个体注销只能改老接口签名并新增按 token 注销的虚函数，为零调用方的接口付第二次 ABI 破坏不值当。多使用方场景应使用 `...Func` 接口对。残留的未通知路径只剩老接口：它返回 void、不去重，槽位已被占用后的注册被静默丢弃，调用方无从得知，而且这个丢弃永远不会波及 `...Func` 的登记表。`...Func` 的超限（32）与基类空实现两种失败都已通过返回 false 暴露，仓外只 override 老虚函数的 `SurfaceBuffer` 派生类或 mock 不再静默失败；代价是 `...Func` 的调用方必须检查返回值，false 时回滚依赖该回调的状态，否则那条状态永远等不到通知。把这两个虚函数的返回类型从 void 改成 bool 是破坏性 ABI 变更：vtable 槽位不变，但旧实现不写返回值寄存器，按新头文件编译的调用方会读到垃圾值而误判失败，因此 graphic_surface 与调用方必须同步编译发布；反向（旧调用方调新实现）安全，忽略返回值即可。带身份的 `...Func` 注册总数上限为 `MAX_BUFFER_DTOR_CB_NUM`（32），防止重复注册导致向量无限增长，达到上限即被拒绝；老接口不受这个上限约束，它只有 master 上那一个槽位，天然不可能无界累积。当前使用方是 graphic_2d 的 `RSBufferReclaim` 和 `RsDelegateCompositeCallbackManager`，两者都已迁移到带身份的 `...Func` 接口并检查返回值，因此 reclaim/resume 不会再连带清掉 delegate composite 的注册。`DoBufferReclaim` 先注册回调再 `TryReclaim`，让清理钩子的存在期完整覆盖 reclaim 过程，与 `DoBufferResume` 把注销放在 `TryResumeIfNeeded` 成功之后形成对称；注册返回 false 就回滚 `bufferReclaimNumsSet_` 记录并直接返回 false 不再 reclaim，避免出现“已回收 + 集合有记录 + 无回调”的记录永久残留、累积到 `MAX_BUFFER_RECLAIM_NUMS`（50）后让整个回收功能失效；reclaim 失败则注销回调并回滚记录。注册必须放在两处提前返回（重复 reclaim、集合已满）之后，否则那两条路径会留下没人清理的注册。`OnCleanCacheForBufferInfoMap` 在注册失败时回退到 `AddBufferReleaseInfo` 立即通知释放，再 `continue` 跳过 `PrepareBufferReleaseInfo`；两条路必须走通其中一条，否则 `currentSurfaceNodeBufferReleaseInfoMapTmp_` 里的记录永远不会被 `ProcessBufferReleaseInfo` 消费。仓内已无 `RegisterBufferDestructorCallback(std::function)` 的调用方，该接口保留给带捕获 lambda 的使用方（无法退化为普通函数指针的场景），代价是这类注册只有一个槽位、无法去重、也无法按注册个体注销，只能由无参注销整体清掉这一个槽位。
