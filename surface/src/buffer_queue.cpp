    ...
    struct IBufferProducer::RequestBufferReturnValue &retval, std::unique_lock<std::mutex> &lock)
{
    ...
    ret = PopFromFreeList(buffer, updateConfig);
    if (ret == GSERROR_OK) {
        return ReuseBuffer(updateConfig, bedata, retval);
    }
    ...
    ret = AllocBuffer(buffer, updateConfig);
    if (ret == GSERROR_OK) {
                                   ...
                                   int64_t expectPresentTimestamp, bool isUsingAutoTimestamp)
{
    SURFACE_TRACE_NAME_FMT("AcquireBuffer with PresentTimestamp name: %s queueId: %" PRIu64 " queueSize: %u"
        "expectPresentTimestamp: %" PRId64, name_.c_str(), uniqueId_, queueSize_, expectPresentTimestamp);
