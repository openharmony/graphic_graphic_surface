...
* Rank: Important(2)
* EnvConditions: N/A
* CaseDescription: 1. preSetup: produce surface flush 3 buffer with timestamp{now, now, now}
*                  2. operation: acqire buffer 3 times with timestamp {now-1, now, now}
*                  3. result: acquire buffer {failed, success, failed} and return {GSERROR_NO_BUFFER_READY, GSERROR_OK,
*                             GSERROR_NO_BUFFER}
...
HWTEST_F(SurfaceIPCWithPTSTest, BufferIPC001, TestSize.Level0)
{
    ...
    //Branch1 - No buffer ready
    auto sRet = cSurface->AcquireBuffer(returnValue, desiredPresentTimestamp - 1, false);
    EXPECT_EQ(sRet, GSERROR_NO_BUFFER_READY);

    //Branch2 - Drop one buffer and acquire one buffer
