...
* Rank: Important(2)
* EnvConditions: N/A
* CaseDescription: test SetAdaptiveFOVMetadata and GetAdaptiveFOVetadata
*/
HWTEST_F(MetadataManagerTest, HDRDynamicMetadataTest, Function | SmallTest | Level2)
{
    ...
    std::vector<uint8_t> metadataGet;
    auto retGet = MetadataHelper::GetAdaptiveFOVetadata(buffer_, metadataGet);
    ASSERT_TRUE(retGet == GSERROR_OK || retSet == GSERROR_HDI_ERROR);
    ...
    ASSERT_EQ(MetadataHelper::GGetAdaptiveFOVetadata(nullBuffer_, metadataGet), GSERROR_NO_BUFFER);
}
}